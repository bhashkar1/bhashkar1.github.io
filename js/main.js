(function(){
  ...
  var mx = -500, my = -500;   /* off screen until mouse enters */
  var rx = -500, ry = -500;   /* smooth ring follower */
  var hue = 0;
  var visible = false;
  var particles = [];
  var prevMx = -500, prevMy = -500;

  /* Track mouse on WINDOW not document — catches all movement */
  ...
  /* Main animation loop — runs every frame regardless of mouse */
  function loop(){
    requestAnimationFrame(loop);
    ctx.clearRect(0, 0, W, H);

    /* hue advances EVERY frame — color always changing */
    hue = (hue + 1.2) % 360;

    /* smooth ring eases toward real cursor */
    rx += (mx - rx) * 0.10;
    ry += (my - ry) * 0.10;
    ...
    if (visible && speed > 0.8) {
      ...
      for (var i = 0; i < count; i++) {
        particles.push(new Particle(mx, my, dx, dy, hue));
      }
    }
    ...
    /* draw cursor — always visible once mouse enters */
    if (visible) {
      /* outer ring — smooth follower, color cycles every frame */
      ctx.beginPath();
  ...
  if (vs) {
    ...
    function dv(){
      ...
      L.forEach(function(l){
        ...
        for(var x=0;x<=vW;x++){
          var e=en(x), y=26+Math.sin(x*l.f+l.p)*l.a*e+Math.sin(x*l.f*2.1+l.p*.55)*l.a*.32*e;
  ...
  function type(){
    ...
    if(!isDel){
      ...
      if(ci===cur.length){ pau=true; isDel=true; setTimeout(type,1950); return; }
      setTimeout(type,44);
    } else {
      tel.textContent = cur.slice(0,--ci);
      if(ci===0){ isDel=false; pi=(pi+1)%ph.length; setTimeout(type,350); return; }
      setTimeout(type,24);
    }
  }
  setTimeout(type, 800);
  ...
  if(hbtn && mm){
    hbtn.addEventListener('click', function(){
      ...
      var sp = hbtn.querySelectorAll('span');
      if(o){ sp[0].style.transform='translateY(6.5px) rotate(45deg)'; sp[1].style.opacity='0'; sp[2].style.transform='translateY(-6.5px) rotate(-45deg)'; }
